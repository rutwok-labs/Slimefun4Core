...
public class GEOMiner extends SlimefunItem implements RecipeDisplayItem, EnergyNetComponent, InventoryBlock, HologramOwner, MachineProcessHolder<GEOMiningOperation> {
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (GEOResource resource : Slimefun.getRegistry().getGEOResources().values()) {
            if (resource.isObtainableFromGEOMiner()) {
                displayRecipes.add(SF4Items.create(resource.getItem(), ChatColor.RESET + resource.getName()));
            }
        }
    ...
    protected void constructMenu(@Nonnull BlockMenuPreset preset) {
        ...
        preset.addItem(4, SF4Items.create(Material.BLACK_STAINED_GLASS_PANE, " "), ChestMenuUtils.getEmptyClickHandler());
    ...
    protected void tick(@Nonnull Block b) {
        ...
        if (operation != null) {
            if (!operation.isFinished()) {
                ...
                inv.replaceExistingItem(4, SF4Items.create(Material.BLACK_STAINED_GLASS_PANE, " "));
